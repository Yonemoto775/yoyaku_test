// Google Apps Script用のWeb API版コード（CORS最終対応版）
...
// OPTIONSリクエストの処理（CORSプリフライトリクエスト）
...
function getBusySlots() {
  try {
    console.log('getBusySlots called');
    ...
    console.error('Error in getBusySlots:', e);
...
function createReservation(data) {
  try {
    ...
    console.log('Checking for overlapping events from', startTime, 'to', endTime);
    // ダブルブッキングの最終チェック
    const overlappingEvents = calendar.getEvents(startTime, endTime);
    const conflictingEvents = overlappingEvents.filter(e => !e.isAllDayEvent());
    ...
    console.log('Recording to spreadsheet');
    ...
    console.error("予約作成中にエラーが発生しました:", e);
...
function getMenuItems() {
  try {
    ...
    console.log('Menu data from sheet:', values);
    ...
    console.error("getMenuItemsでエラーが発生しました:", e);
    return [
      { name: 'ハンドケア', duration: 60, price: 5000 },
      { name: 'ジェルネイル', duration: 90, price: 8000 },
      { name: 'スカルプチュア', duration: 120, price: 12000 }
...
function sendSalonNotification(data, formattedStartTime, basePrice, extensionPrice, staffAssignmentPrice, totalPrice, imageFileUrl) {
  console.log('sendSalonNotification called');
  ...
  try {
    ...
    notificationEmails.forEach(email => {
      try {
        ...
        console.error(`通知メールの送信に失敗しました (${email}):`, e);
      }
    });
    
  } catch (e) {
    console.error("サロン側への通知メール送信に失敗しました:", e);
  }
}